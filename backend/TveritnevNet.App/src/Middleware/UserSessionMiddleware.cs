...
public abstract class UserSessionMiddleware(IDatabaseConnectionFactory databaseConnectionFactory) : IMiddleware
{
    ...
    {
        if (!(await _sessionRepository.WhereOneAsync("token", request.GetCookie("Token"), cancellationToken)).Out(
               out var sessionDatabaseModel)) return Option.Fail();

        if (!(await _userRepository.WhereOneAsync("id", sessionDatabaseModel.UserId, cancellationToken)).Out(
