...
public sealed class UsersController(IDatabaseConnectionFactory databaseConnectionFactory) : Controller {
   ...
   private async Task UsersMeHandler(Request request, Response response, CancellationToken cancellationToken) {
      ...
      response.WriteTJsonToCache(new {
         Username = usersDatabaseModel.Username, Password = usersDatabaseModel.Password, Role = usersDatabaseModel.Role
      });
   }

   [ControllerHandler("/users/me/role", HttpMethodType.GET, typeof(ModifyLoggerMiddleware),
   ...
   [ControllerHandler("/users/me/username", HttpMethodType.GET, typeof(ModifyLoggerMiddleware),
      typeof(MemberSessionMiddleware))]
   ...
   [ControllerHandler("/users/me/id", HttpMethodType.GET, typeof(ModifyLoggerMiddleware),
      typeof(MemberSessionMiddleware))]
   ...
   private async Task UsersLoginHandler(Request request, Response response, CancellationToken cancellationToken) {
      if (!(await WebApp.Context.Validator.ValidFromRequest<LoginModel>(request, response, cancellationToken)).Out(
             out var loginModel)) return;

      if (!(await _userRepository.FirstWhere("username", loginModel.Username, cancellationToken)).Out(
      ...
      if (request.GetCookie("Token").Out(out var token)) {
         if (!(await _sessionRepository.FirstWhere("user_id", usersDatabaseModel.Id, cancellationToken)).Out(
   ...
   [ControllerHandler("/users/change_password", HttpMethodType.POST, typeof(ModifyLoggerMiddleware),
      typeof(MemberSessionMiddleware))]
   private async Task UsersChangePasswordHandler(Request request, Response response,
      CancellationToken cancellationToken) {
      if (!(await WebApp.Context.Validator.ValidFromRequest<ChangePasswordModel>(request, response, cancellationToken))
   ...
   [ControllerHandler("/users/logout", HttpMethodType.POST, typeof(ModifyLoggerMiddleware),
      typeof(MemberSessionMiddleware))]
