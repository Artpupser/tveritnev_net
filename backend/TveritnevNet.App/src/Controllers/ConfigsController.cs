...
public sealed class ConfigsController : Controller {
   ...
   [ControllerHandler("/configs/default", HttpMethodType.GET, typeof(ModifyLoggerMiddleware),
   ...
   private async Task
      ...
      if (!(await _configsRepository.FirstWhere("name", $"default_{configLoadModel.Name}", cancellationToken)).Out(
             out var configsDatabaseModel)) {
         response.PushError("Error load config from storage");
         return;
      }
   ...
   [ControllerHandler("/configs/save", HttpMethodType.POST, typeof(ModifyLoggerMiddleware),
      typeof(AdminSessionMiddleware))]
   private async Task ConfigsSavePanelHandler(Request request, Response response, CancellationToken cancellationToken) {
      if (!(await WebApp.Context.Validator.ValidFromRequest<ConfigSaveModel>(request, response, cancellationToken)).Out(
      ...
      if (await _configsRepository.ChangeFrom("name", configSaveModel.Name, "json", configSaveModel.Json,
             cancellationToken)) {
